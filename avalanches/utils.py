...
def plot_plant(t0, x0, species, tmax, R, a, cols, ax):
    ...
    ts = np.linspace(0, tmax[s], 100)+t0
    Rs = growth(ts, t0, s, R, a) 
...
def plot_schema_optimal(srs, eta, x_p, prs, pvs, pxs, pss, accept, svg, i, m, t, xmax=10):
   ...
   pl.subplot(211)
   pl.axis("off")
   pl.plot([0,xmax],[0,0], "k")
   pl.plot([0,xmax],[1,1], "k")
   pl.plot([0,0],[0,1], "k")
   pl.plot([xmax,xmax],[0,1], "k")
   pl.fill_between([0, srs], [0, 0], [1,1], color=[.1,.6,.1])
   pl.fill_between([srs, xmax], [0, 0], [1,1], color="#805b21")
   pl.fill_between([0, svs], [0, 0], [1,1], color=[.1,.6,.1], alpha=.3)
   pl.plot([svs+eta,svs+eta], [0,1], color ="k", lw=4)
   pl.xlim([0,xmax]) 
   pl.text(svs/2, .5, r'$\sum_{i=1}^N \sigma_i$', fontsize=24)
   pl.text(svs+2, .5, r'$\phi_{i,N}$', fontsize=24)
   pl.text(svs+eta+.1, .8, r'$\eta$', fontsize=24)
   pl.title("Plant %s Trial %s Time %.02f"%(i, m, t))
   pl.subplot(212)
   #pl.axis("off")
   pl.fill_between([0, xmax], [0, 0], [1,1], color="#805b21")
   pl.plot([0,xmax],[0,0], "k")
   pl.plot([0,xmax],[1,1], "k")
   pl.plot([0,0],[0,1], "k")
   pl.plot([xmax,xmax],[0,1], "k")
   ...
   for i in range(len(prs)):
      pl.fill_between([pxs[i]-prs[i]/2, pxs[i]+prs[i]/2], [0, 0], [1,1], color=cols[pss[i]])
      pl.fill_between([pxs[i]-pvs[i]/2, pxs[i]+pvs[i]/2], [0, 0], [1,1], color=cols[pss[i]], alpha=.3)
   pl.xlim([0,xmax]) 
   pl.xticks(np.linspace(0,100,51))
